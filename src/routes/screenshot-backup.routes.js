...
export function screenshotBackupRouter (app) {
  ...
  app.get(`${path}/summary/:schema`, security.verifyJWT, (req, res) => {
  ...
  app.get(`${path}/summary-all/:schema`, security.verifyJWT, (req, res) => {
    service.getSummaryAll({ schema: req.params.schema })
      .then(data => {
        if (data.length <= 0) res.status(204)
  ...
  app.post(`${path}/auth`, async (req, res) => {
    try {
      ...
      const data = await authService.auth({ name, user, key }, { apiKey: process.env.API_SCREENSHOT_BACKUP_KEY, apiSecret: process.env.API_SCREENSHOT_BACKUP_TOKEN_SECRET })
      res.send(generalResponse.ok(data))
    } catch (error) {
